...
class TestDashboardView:
    ...
    def test_planeacion_renders_best_manual_plan_execution_readiness(self, auth_client, monkeypatch):
        monkeypatch.setattr(
            'apps.dashboard.views.get_planeacion_incremental_context',
            lambda query_params, user, capital_amount=600000, history_limit=5: {
                ...
                'manual_incremental_portfolio_simulation_comparison': {
                    ...
                    'best_execution_readiness': {
                        ...
                        'tone': 'warning',
                        'headline': 'Plan manual A lidera el comparador manual, pero pide validar ejecucion antes de comprar.',
                        'summary': 'Arrancar por KO y dejar MCD para una validacion adicional.',
                    },
                    'proposals': [
                        {
                            'proposal_key': 'plan_a',
        ...
        body = response.content.decode()
        assert response.status_code == 200
        assert 'Lectura operativa del mejor plan manual' in body
    ...
    def test_planeacion_renders_best_candidate_execution_readiness_and_tiebreak(self, auth_client, monkeypatch):
        monkeypatch.setattr(
            'apps.dashboard.views.get_planeacion_incremental_context',
            lambda query_params, user, capital_amount=600000, history_limit=5: {
                'portfolio_scope_summary': {'portfolio_total_broker': 0, 'cash_available_broker': 0, 'cash_settling_broker': 0, 'invested_portfolio': 0, 'cash_management_fci': 0},
                ...
                'candidate_incremental_portfolio_comparison': {
                    ...
                    'best_execution_readiness': {
                        ...
                        'label': 'Listo para ejecutar',
                        'tone': 'success',
                        'headline': 'MCD es el mejor plan manual y hoy queda listo para ejecutar.',
                        'summary': 'Si avanzas con esta propuesta, conviene empezar por MCD.',
                    },
                    'operational_tiebreak': {
                        'has_tiebreak': True,
                        'used_operational_tiebreak': True,
                        'headline': 'Desempate operativo aplicado',
                        'summary': 'MCD queda primero porque su ejecucion real reciente se ve mas limpia dentro de una brecha corta de score.',
                    },
                    'proposals': [
                        {
                            'proposal_key': 'MCD',
        ...
        body = response.content.decode()
        assert response.status_code == 200
        assert 'Lectura operativa del mejor candidato' in body
        assert 'Desempate operativo aplicado' in body
        assert 'MCD queda primero porque su ejecucion real reciente se ve mas limpia dentro de una brecha corta de score.' in body

    def test_planeacion_renders_best_split_execution_readiness_and_tiebreak(self, auth_client, monkeypatch):
        monkeypatch.setattr(
            'apps.dashboard.views.get_planeacion_incremental_context',
            lambda query_params, user, capital_amount=600000, history_limit=5: {
                'portfolio_scope_summary': {'portfolio_total_broker': 0, 'cash_available_broker': 0, 'cash_settling_broker': 0, 'invested_portfolio': 0, 'cash_management_fci': 0},
                ...
                'candidate_split_incremental_portfolio_comparison': {
                    ...
                    'best_execution_readiness': {
                        ...
                        'label': 'Listo para ejecutar',
                        'tone': 'success',
                        'headline': 'Split KO + MCD es el mejor plan manual y hoy queda listo para ejecutar.',
                        'summary': 'Arrancar por KO y dejar MCD para una validacion adicional.',
                    },
                    'operational_tiebreak': {
                        'has_tiebreak': True,
                        'used_operational_tiebreak': True,
                        'headline': 'Desempate operativo aplicado',
                        'summary': 'Split KO + MCD queda primero porque su ejecucion real reciente se ve mas limpia dentro de una brecha corta de score.',
                    },
                    'proposals': [
                        {
        ...
        body = response.content.decode()
        assert response.status_code == 200
        assert 'Lectura operativa de la mejor construcción' in body or 'Lectura operativa de la mejor construccion' in body
        assert 'Desempate operativo aplicado' in body
    ...
    def test_planeacion_renders_execution_readiness_per_row_in_general_comparator(self, auth_client, monkeypatch):
        monkeypatch.setattr(
            'apps.dashboard.views.get_planeacion_incremental_context',
            lambda query_params, user, capital_amount=600000, history_limit=5: {
                'portfolio_scope_summary': {'portfolio_total_broker': 0, 'cash_available_broker': 0, 'cash_settling_broker': 0, 'invested_portfolio': 0, 'cash_management_fci': 0},
                ...
                'incremental_portfolio_simulation_comparison': {
                    'best_label': 'Top candidato por bloque',
                    'best_proposal_key': 'top_candidate_per_block',
                    'proposals': [
                        {
                            'proposal_key': 'top_candidate_per_block',
                            'proposal_label': 'Top candidato por bloque',
                            'label': 'Top candidato por bloque',
                            'comparison_score': 3.1,
                            'selected_candidates': [{'symbol': 'KO', 'amount': 600000}],
                            'simulation': {'delta': {'expected_return_change': 0.4, 'fragility_change': -1.5, 'scenario_loss_change': 0.3}, 'interpretation': 'Top candidato por bloque.'},
                            'execution_readiness': {'label': 'Listo para ejecutar', 'tone': 'success', 'summary': 'La propuesta automatica ya tiene fit operativo suficiente para ejecutarse.'},
                        },
                        {
                            'proposal_key': 'runner_up_when_available',
                            'proposal_label': 'Segundo candidato si existe',
                            'label': 'Segundo candidato si existe',
                            'comparison_score': 2.7,
                            'selected_candidates': [{'symbol': 'PEP', 'amount': 600000}],
                            'simulation': {'delta': {'expected_return_change': 0.3, 'fragility_change': -1.0, 'scenario_loss_change': 0.2}, 'interpretation': 'Runner up.'},
                            'execution_readiness': {'label': 'Validar ejecucion', 'tone': 'warning', 'summary': 'La variante todavia tiene poca evidencia operativa comparable.'},
                        },
                    ],
                },
                'candidate_incremental_portfolio_comparison': {'available_blocks': [], 'proposals': [], 'best_label': None},
        ...
        assert 'Listo para ejecutar' in body
        assert 'Validar ejecucion' in body or 'Validar ejecuci' in body
        assert 'fit operativo suficiente para ejecutarse' in body
        assert 'poca evidencia operativa comparable' in body

    def test_planeacion_renders_readiness_filter_summaries_in_incremental_comparators(self, auth_client, monkeypatch):
        monkeypatch.setattr(
            'apps.dashboard.views.get_planeacion_incremental_context',
            lambda query_params, user, capital_amount=600000, history_limit=5: {
                'portfolio_scope_summary': {'portfolio_total_broker': 0, 'cash_available_broker': 0, 'cash_settling_broker': 0, 'invested_portfolio': 0, 'cash_management_fci': 0},
                ...
                'incremental_portfolio_simulation_comparison': {
                    ...
                    'best_proposal_key': 'top_candidate_per_block',
                    'proposals': [],
                    'available_readiness_filters': [{'value': 'all', 'label': 'Todas', 'is_selected': False}, {'value': 'ready', 'label': 'Listo para ejecutar', 'is_selected': True}],
                    'has_active_readiness_filter': True,
                    'active_readiness_filter_label': 'Listo para ejecutar',
                    'visible_count': 1,
                    'total_count': 3,
                },
                'candidate_incremental_portfolio_comparison': {
                    ...
                    'selected_label': 'Defensive / resiliente',
                    'proposals': [],
                    'available_readiness_filters': [{'value': 'all', 'label': 'Todas', 'is_selected': False}, {'value': 'review_execution', 'label': 'Validar ejecucion', 'is_selected': True}],
                    'has_active_readiness_filter': True,
                    'active_readiness_filter_label': 'Validar ejecucion',
                    'visible_count': 1,
                    'total_count': 2,
                },
                'candidate_split_incremental_portfolio_comparison': {
                    ...
                    'selected_label': 'Defensive / resiliente',
                    'proposals': [],
                    'available_readiness_filters': [{'value': 'all', 'label': 'Todas', 'is_selected': False}, {'value': 'monitor', 'label': 'Seguir observando', 'is_selected': True}],
                    'has_active_readiness_filter': True,
                    'active_readiness_filter_label': 'Seguir observando',
                    'visible_count': 1,
                    'total_count': 2,
                },
                'manual_incremental_portfolio_simulation_comparison': {
        ...
        body = response.content.decode()
        assert response.status_code == 200
        assert 'Mostrando 1 de 3 propuestas con estado' in body
    ...
    def test_planeacion_renders_active_comparator_summary(self, auth_client, monkeypatch):
        monkeypatch.setattr(
            'apps.dashboard.views.get_planeacion_incremental_context',
            lambda query_params, user, capital_amount=600000, history_limit=5: {
                'portfolio_scope_summary': {'portfolio_total_broker': 0, 'cash_available_broker': 0, 'cash_settling_broker': 0, 'invested_portfolio': 0, 'cash_management_fci': 0},
                ...
                'candidate_asset_ranking': {'candidate_assets': [], 'candidate_assets_count': 0, 'by_block': {}, 'explanation': ''},
                'incremental_portfolio_simulation': {'delta': {}, 'interpretation': ''},
                'incremental_portfolio_simulation_comparison': {'proposals': [], 'available_readiness_filters': []},
                'candidate_incremental_portfolio_comparison': {'available_blocks': [], 'proposals': []},
                'candidate_split_incremental_portfolio_comparison': {'available_blocks': [], 'proposals': []},
                'manual_incremental_portfolio_simulation_comparison': {'submitted': False, 'proposals': [], 'form_state': {}, 'available_readiness_filters': []},
                'incremental_comparator_form_state': {
                    'general_hidden_inputs': [], 'general_reset_url': '/planeacion/#planeacion-aportes',
                    'candidate_hidden_inputs': [], 'candidate_reset_url': '/planeacion/#planeacion-aportes',
                    'split_hidden_inputs': [], 'split_reset_url': '/planeacion/#planeacion-aportes',
                    'manual_hidden_inputs': [], 'manual_reset_url': '/planeacion/#planeacion-aportes',
                },
                'incremental_comparator_activity_summary': {
                    'has_active_context': True,
                    'active_count': 3,
                    'headline': '3 comparadores activos.',
                    'items': [
                        {'label': 'Comparador general', 'summary': 'Filtro activo: Listo para ejecutar.'},
                        {'label': 'Comparador por candidato', 'summary': 'Bloque: Defensive / resiliente. Filtro: Validar ejecucion.'},
                        {'label': 'Comparador manual', 'summary': 'Planes manuales cargados. Filtro: Listo para ejecutar.'},
                    ],
                },
                'preferred_incremental_portfolio_proposal': {'preferred': {'proposal_label': 'Plan A'}, 'has_manual_override': False, 'explanation': ''},
                'operation_execution_feature': {'has_context': False, 'has_symbols': False, 'tracked_symbols_count': 0, 'matched_symbols_count': 0, 'missing_symbols_count': 0, 'coverage_pct': 0, 'headline': '', 'summary': '', 'alerts': [], 'execution_analytics': {}, 'rows': []},
        ...
        body = response.content.decode()
        assert response.status_code == 200
        assert 'Estado activo de comparadores' in body
        assert '3 comparadores activos.' in body
        assert 'Comparador general' in body
    ...
    def test_planeacion_renders_execution_quality_rows_inside_preferred_proposal(self, auth_client, monkeypatch):
        monkeypatch.setattr(
            'apps.dashboard.views.get_planeacion_incremental_context',
            lambda query_params, user, capital_amount=600000, history_limit=5: {
                'portfolio_scope_summary': {'portfolio_total_broker': 0, 'cash_available_broker': 0, 'cash_settling_broker': 0, 'invested_portfolio': 0, 'cash_management_fci': 0},
                ...
                'manual_incremental_portfolio_simulation_comparison': {'submitted': False, 'proposals': [], 'form_state': {}},
                'preferred_incremental_portfolio_proposal': {'preferred': {'proposal_label': 'Plan A'}, 'has_manual_override': False, 'explanation': ''},
                'operation_execution_feature': {'has_context': True, 'has_symbols': True, 'tracked_symbols_count': 2, 'matched_symbols_count': 1, 'missing_symbols_count': 1, 'coverage_pct': 50, 'headline': '', 'summary': '', 'alerts': [], 'execution_analytics': {'fee_visible_pct': 100, 'fee_visible_count': 1, 'fragmented_pct': 0, 'fragmented_count': 0}, 'rows': []},
                'decision_engine_summary': {
                    'score': 61,
                    'confidence': 'Media',
                    'explanation': [],
                    ...
                    'preferred_proposal': {
                        ...
                        'source_label': 'Comparador manual',
                        'purchase_summary': 'KO · 300000, MCD · 300000',
                    ...
                    'expected_impact': {'summary': 'Impacto incremental no disponible.'},
                    'operation_execution_signal': {'has_signal': True, 'status': 'partial', 'title': '', 'summary': ''},
                    'execution_gate': {'has_blocker': False, 'primary_cta_label': 'Ejecutar decisión', 'primary_cta_tone': 'success'},
                    'tracking_payload': {'purchase_plan': [{'symbol': 'KO', 'amount': 300000}], 'score': 61, 'confidence': 'Media'},
                },
                'incremental_proposal_history': {'items': [], 'count': 0, 'has_history': False, 'active_filter': 'all', 'active_filter_label': 'Todos', 'decision_counts': {'total': 0, 'pending': 0, 'accepted': 0, 'deferred': 0, 'rejected': 0}, 'available_filters': [], 'headline': ''},
                'incremental_proposal_tracking_baseline': {'item': None, 'has_baseline': False},
                'incremental_backlog_prioritization': {'count': 0, 'has_priorities': False, 'has_shortlist': False, 'has_focus_split': False, 'counts': {'high': 0, 'medium': 0, 'watch': 0, 'low': 0}, 'followup_counts': {'review_now': 0, 'monitor': 0, 'hold': 0}, 'active_followup_filter': 'all', 'active_followup_filter_label': 'Todas', 'available_followup_filters': [], 'manual_review_summary': {'pending_count': 0, 'deferred_count': 0, 'accepted_count': 0, 'rejected_count': 0, 'closed_count': 0, 'reviewed_count': 0, 'headline': '', 'has_manual_reviews': False}, 'deferred_review_summary': {'deferred_count': 0, 'reactivable_count': 0, 'archivable_count': 0, 'top_reactivable_label': '', 'top_reactivable_priority_label': '', 'has_reactivable': False, 'headline': ''}, 'headline': '', 'explanation': '', 'top_item': None, 'economic_leader': None, 'tactical_leader': None},
