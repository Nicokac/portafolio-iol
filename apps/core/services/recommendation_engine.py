...
from apps.core.services.recommendation_signal_support import (
    ...
    extract_signal_assets,
    map_signal_to_recommendation,
    prioritize_recommendations,
    recommendation_sort_key,
    recommendation_specificity_rank,
...
class RecommendationEngine:
    ...
    def _analyze_analytics_v2(self) -> List[Dict]:
        try:
            return build_analytics_v2_recommendations(
                self,
                ScenarioAnalysisService,
                FactorExposureService,
                StressFragilityService,
    ...
    def _build_risk_contribution_signals(self) -> List[Dict]:
        return build_risk_contribution_signals(
            logger,
            CovarianceAwareRiskContributionService,
            RiskContributionService,
        )
    ...
    def _prioritize_recommendations(self, recommendations: List[Dict]) -> List[Dict]:
        return prioritize_recommendations(recommendations, self.PRIORITY_RANK)

    @staticmethod
    def _recommendation_topic_key(recommendation: Dict) -> str:
        return recommendation_topic_key(recommendation)

    def _recommendation_sort_key(self, recommendation: Dict, index: int) -> tuple:
        return recommendation_sort_key(recommendation, index, self.PRIORITY_RANK)

    @staticmethod
    def _recommendation_specificity_rank(recommendation: Dict) -> int:
        return recommendation_specificity_rank(recommendation)

    def _map_signal_to_recommendation(self, signal: Dict) -> Dict:
        return map_signal_to_recommendation(self, signal)

    @staticmethod
    def _extract_signal_assets(evidence: Dict) -> List[str]:
        return extract_signal_assets(evidence)

    @staticmethod
    def _build_signal_actions(signal: Dict) -> List[str]:
        return build_signal_actions(signal)
